...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!--
        Generate a RSS-feed, dynamically.
    ...
    <xsl:import href="../utilities/blog-format-date.xsl" />
    ...
    <xsl:template name="rss">
        ...
        <rss version="2.0">
            <channel>
                ...
                <atom:link href="http://fetimo.com/blog/rss-feed/" rel="self" type="application/rss+xml" />
                <link>http://fetimo.com/blog/rss-feed/</link>
                ...
                <xsl:for-each select="$items">
                    <item>
                        ...
                        <title><xsl:value-of select="*[name() = $item-titlenode]" /></title>
                        <!--<link><xsl:value-of select="$item-link-processed" /></link>-->
                        <link>http://fetimo.com/blog/post/<xsl:value-of select="dyn:evaluate($item-link-part)"/></link>
                        <pubDate>
                            <xsl:call-template name="format-date">
                                <xsl:with-param name="date" select="*[name() = $item-datenode]" />
                                <xsl:with-param name="format" select="'%d-;, %0d; %m-; %y+; #0H;:#0m;:#0s;'" />
                            </xsl:call-template>
                            <xsl:value-of select="concat(' ', $timezone)" />
                        </pubDate>
                        <guid>http://fetimo.com/blog/post/<xsl:value-of select="dyn:evaluate($item-link-part)"/></guid>
                        <description><xsl:value-of select="*[name() = $item-contentnode]" /></description>
                    </item>
                </xsl:for-each>
            </channel>
        </rss>
