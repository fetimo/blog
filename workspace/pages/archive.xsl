...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="data" mode="main">		
		<section class="accompanied">
			<xsl:for-each select="archive-posts/year/month/entry">
				...
				<article>
					...
					<em>
						...
						<xsl:choose>
							<xsl:when test="count(../../../../comments/entry/commented-on/item[@handle=$post-title]) = 0">
								there are no comments (yet!).
							</xsl:when>
							<xsl:when test="count(../../../../comments/entry/commented-on/item[@handle=$post-title]) = 1">
								there is 1 comment.
							</xsl:when>
							<xsl:otherwise>
								there are <xsl:value-of select="count(../../../../comments/entry/commented-on/item[@handle=$post-title])"/> comments.
							</xsl:otherwise>
						</xsl:choose>
					</em>
				</article>
